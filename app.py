...
@app.route("/correctionUpload", methods=["POST"])
def upload_image_correction():
    if "image" not in request.files:
        return jsonify({"error": "No image uploaded"}), 400
...
pannum.value = "{variables['pan_card_number']}";
pannum.dispatchEvent(new Event("change",{{ bubbles: true }} ));
